...
def create_folder(json_data):  # 创建文件夹
    ...
    anime_location_rule = eval(conf.anime_location_rule(),json_data)
    ...
    if 'title' in conf.anime_location_rule() and len(title) > maxlen:
        shorttitle = title[0:maxlen]
        anime_location_rule = anime_location_rule.replace(title,shorttitle)
    path = ''
    # 当演员为空时，location_rule被计算为'/number'绝对路径，导致路径连接忽略第一个路径参数，因此添加./使其始终为相对路径
    if animeflag:
        path = os.path.join(success_folder, f'./{anime_location_rule.strip()}')
    else:
    ...
    if not os.path.exists(path):
        ...
        try:
            ...
            if animeflag:
                path = success_folder + '/' + anime_location_rule.replace('/[' + number + ')-' + title, "/number")
            else:
...
def extrafanart_download(data, path, number, filepath, json_data=None, animeflag = False):
    ...
    extrafanart_download_one_by_one(data, path, filepath, json_data, number)
...
def print_files(path, leak_word, c_word, naming_rule, anime_naming_rule, part, cn_sub, json_data, filepath, tag, actor_list, liuchu,
    ...
    try:
        ...
        with open(nfo_path, "wt", encoding='UTF-8') as code:
            '''
            单独给动画打印格式，不打印番号，只在outline显示番号
            '''
            print('<?xml version="1.0" encoding="UTF-8" ?>', file=code)
            print("<movie>", file=code)
            if not config.getInstance().jellyfin():
                if animeflag:
                    print("  <title><![CDATA[" + anime_naming_rule + "]]></title>", file=code)
                    print("  <originaltitle><![CDATA[" + anime_naming_rule + "]]></originaltitle>",
                      file=code)
                    print("  <sorttitle><![CDATA[" + title + "]]></sorttitle>", file=code)    
                else:
                ...
                if animeflag:
                    print("  <title>" + anime_naming_rule + "</title>", file=code)
                    print("  <originaltitle>" + anime_naming_rule + "</originaltitle>", file=code)
                    print("  <sorttitle>" + anime_naming_rule + "</sorttitle>", file=code)
                else:
            ...
            outline = outline.replace(number,"")
            if not config.getInstance().jellyfin():
                if animeflag:
                    print("  <outline><![CDATA["+animenumber+" "+ outline + "]]></outline>", file=code)
                    print("  <plot><![CDATA[" +animenumber+" "+ outline + "]]></plot>", file=code)
                else:
                    print("  <outline><![CDATA["+number+" "+ outline + "]]></outline>", file=code)
                    print("  <plot><![CDATA[" +number+" "+ outline + "]]></plot>", file=code)
        


            else:
                if animeflag:
                    print("  <outline>"+ animenumber +" "+ outline + "</outline>", file=code)
                    print("  <plot>" +animenumber+" "+ outline + "</plot>", file=code)
                else:
...
def core_main(movie_path, number_th, oCC, specified_source=None, specified_url=None):
    ...
    if conf.main_mode() == 1:
        ...
        if conf.is_extrafanart() and json_data.get('extrafanart'):
                if 'headers' in json_data:
                    extrafanart_download(json_data.get('extrafanart'), path, number, movie_path, json_data,animeflag=animeflag)
                else:
                    extrafanart_download(json_data.get('extrafanart'), path, number, movie_path,animeflag=animeflag)
        # 裁剪图
...
def getPreviewImageUrlFromBlogJAV(url):
    ...
    image_list = data.xpath('/html/body/div[1]/div/div[1]/main/article/div/div/p[1]/a/img/@data-lazy-src')
    if len(image_list) >0:
        return image_list[0].replace('thumbs', 'images').replace('/t', '/img')
    return ""
