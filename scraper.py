...
def get_data_from_json(
   ...
   # if conf.number_uppercase():
    ...
    for i in conf.anime_naming_rule().split("+"):
        if i not in json_data:
            anime_naming_rule += i.strip("'").strip('"')
        else:
            item = json_data.get(i)
            anime_naming_rule += item if type(item) is not list else "&".join(item)

    json_data['naming_rule'] = naming_rule
    json_data['original_naming_rule'] = original_naming_rule
    json_data['anime_naming_rule'] = anime_naming_rule
    return json_data
